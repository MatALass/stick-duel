...
class MeleeStartupState(FighterState):
    name = "melee_startup"
    interruptible = False
    ...
    def update(self, fighter, dt: float) -> None:
        fighter.apply_horizontal_input(acceleration=0.25)
        self.remaining -= dt
        if self.remaining <= 0:
            fighter.state_machine.change_state(fighter.state_factory.melee_active())
